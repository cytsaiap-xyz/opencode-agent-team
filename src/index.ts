...
export const AgentTeamPlugin: Plugin = async (ctx) => {
    ...
    return {
        ...
        tool: {
            ...
            delegate_task: tool({
                description:
                    "Delegate a task to a subagent from the current team. " +
                    "Creates a child session with the agent's system prompt and permissions, " +
                    "sends the task, waits for completion, and returns the result.",
                args: {
                ...
                async execute(args) {
                    ...
                    try {
                        ...
                        // Use worker agent with custom system prompt override
                        ...
                        console.log(`[agent-team] Task dispatched to worker session ${sessionId}`);
                        ...
                        return `[Agent ${args.agent_id}] Task dispatched (session ${sessionId}), may still be running.`;
                    } catch (error) {
